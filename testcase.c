...
int testcase1(){
    ...
    for (i = 0;i < 32;i++) {
        if ((pBuf = BufFind(i)) != NULL && (pBuf->state == BUF_STATE_DIRTY))
            BufSyncBlock(i);
...
void PrintBufInfo()
{
    ...
    Buf*    ppBufInfo[MAX_BUF_NUM];
        ...
        for (i = 0 ; i < MAX_BUFLIST_NUM; i++) {
            numBuf = GetBufInfoInBufferList(i, ppBufInfo, MAX_BUF_NUM);
    ...
    memset( ppBufInfo, 0, (MAX_BUF_NUM * sizeof(Buf*)) );
    numBuf = GetBufInfoInStateList(BUF_DIRTY_LIST, ppBufInfo, MAX_BUF_NUM);

    printf( "Num of Bufs in the BUF_DIRTY_LIST : %d\n", numBuf );
    for( i = 0; i < numBuf; i++ ){
    ...
    memset( ppBufInfo, 0, (MAX_BUF_NUM * sizeof(Buf*)) );
    numBuf = GetBufInfoInStateList(BUF_CLEAN_LIST, ppBufInfo, MAX_BUF_NUM);

    printf( "Num of Bufs in the BUF_CLEAN_LIST : %d\n", numBuf );
    for( i = 0; i < numBuf; i++ ){
        printf( "Buf[%d,", ppBufInfo[i]->blkno );
        if ( ppBufInfo[i]->state == BUF_STATE_CLEAN )
        {
    ...
    memset( ppBufInfo, 0, (MAX_BUF_NUM * sizeof(Buf*)) );
    numBuf= GetBufInfoInLruList( ppBufInfo, MAX_BUF_NUM);
